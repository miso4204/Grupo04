...
import com.thinktanksoft.stampurestyle.Talla;
...
public class TallaFacadeREST extends AbstractFacade<Talla> {
    ...
    @Path("{id}")
    @Consumes({"application/xml", "application/json"})
    public void edit(@PathParam("id") Integer id, Talla entity) {
        super.edit(entity);
    }
    ...
    public Talla find(@PathParam("id") Integer id) {
    ...
    @Path("{from}/{to}")
    @Produces({"application/xml", "application/json"})
    public List<Talla> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
